...
class ScoringDataTable extends DataTable
{
     ...
     */
    public function query(ExamScore $model): QueryBuilder
    {
        return $model->newQuery()
                    ->select([
                        'exam_scores.*',
                        DB::raw('er.exam_date AS exam_date'),
                        DB::raw('er.exam_time AS exam_time'),
                        DB::raw('u.name AS mahasiswa'),
                        DB::raw('et.name AS ujian'),
                        DB::raw('lec.name AS dosen'),
                        DB::raw('ch.name AS ketua'),
                        DB::raw("CONCAT(er.exam_date, ' ', er.exam_time) AS waktu"),
                    ])
                    ->join('exam_registrations AS er', 'exam_scores.exam_registration_id', '=', 'er.id')
