...
>
    {{-- Desktop collapse/expand toggle lives in the topbar now (merged next to the
         logo, see filament-panels/components/topbar/index.blade.php). The sidebar
         keeps its own logo-only header row, shown while the sidebar is open, mirroring
         the Filament v4 "fi-sidebar-header" layout. --}}
    <div class="fi-sidebar-header-ctn overflow-x-clip">
        <header
        ...
        >
            ...
            {{-- Fallback for the rare case a panel disables the topbar entirely —
                 without it, there would be no way to expand/collapse the sidebar. --}}
            @unless (filament()->hasTopbar())
                @if (filament()->isSidebarCollapsibleOnDesktop())
