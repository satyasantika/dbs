<div wire:poll.30s class="relative pb-1 space-y-4">
@php
...
{{-- Status: warna inline (Tailwind di dalam modal Livewire sering tidak diterapkan) --}}
@if ($total === 0)
    <div style="border-radius:10px;border:1px solid #f59e0b;background:#fffbeb;padding:12px 16px;margin-bottom:16px;color:#92400e;font-size:14px;line-height:1.45;">
        Penguji belum diset — exam_scores belum dibuat.
    </div>
@elseif ($allScored)
    <div style="border-radius:10px;border:1px solid #34d399;background:#ecfdf5;padding:12px 16px;margin-bottom:16px;color:#065f46;font-size:14px;line-height:1.45;display:flex;align-items:flex-start;gap:10px;flex-wrap:wrap;">
        <svg xmlns="http://www.w3.org/2000/svg" style="width:20px;height:20px;flex-shrink:0;margin-top:2px;color:#059669" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        <span style="flex:1;min-width:120px;">Semua penilaian sudah lengkap.</span>
        @if ($record->sent_at)
            <span style="font-size:12px;color:#047857;white-space:nowrap;">Dikabari {{ $record->sent_at->locale('id')->isoFormat('D MMM Y, HH.mm') }}</span>
        @endif
    </div>
@else
    <div style="border-radius:10px;border:1px solid #f87171;background:#fef2f2;padding:12px 16px;margin-bottom:16px;color:#991b1b;font-size:14px;line-height:1.45;display:flex;align-items:flex-start;gap:10px;">
        <svg xmlns="http://www.w3.org/2000/svg" style="width:20px;height:20px;flex-shrink:0;margin-top:2px;color:#dc2626" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/></svg>
        <span>Penilaian belum lengkap — menunggu <strong>{{ $pending }}</strong> penguji.</span>
    </div>
@endif
...
@php
    ...
    $styBtnPrimary = 'display:inline-flex;align-items:center;justify-content:center;gap:10px;background:#059669;color:#ffffff !important;text-decoration:none !important;border-radius:12px;padding:14px 22px;font-weight:700;font-size:13px;line-height:1.25;min-height:42px;box-shadow:0 3px 8px rgba(5,150,105,.35);width:100%;max-width:120px;box-sizing:border-box;border:none;cursor:pointer;';
    $styBtnSecondary = 'display:inline-flex;align-items:center;justify-content:center;gap:8px;background:#475569;color:#ffffff !important;text-decoration:none !important;border-radius:9px;padding:11px 16px;font-weight:700;font-size:13px;line-height:1.25;min-height:42px;box-sizing:border-box;border:none;cursor:pointer;box-shadow:0 1px 4px rgba(0,0,0,.12);';
    $styBtnThesis = 'display:inline-flex;align-items:center;justify-content:center;gap:8px;background:#059669;color:#ffffff !important;text-decoration:none !important;border-radius:9px;padding:11px 16px;font-weight:700;font-size:13px;line-height:1.25;min-height:42px;box-sizing:border-box;border:none;cursor:pointer;box-shadow:0 1px 4px rgba(5,150,105,.25);';
@endphp
<div style="{{ $styToolbarWrap }}">
    <p style="{{ $styToolbarTitle }}">Unduh Berita Acara</p>
    <div style="display:flex;flex-direction:column;gap:14px;">
        <a href="{{ route('report.exam-chief', ['examregistration' => $record->id]) }}?pdf=1" target="_blank" rel="noopener noreferrer" style="{{ $styBtnPrimary }}">
            <span>Hasil Ujian</span>
        </a>
        {{-- <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;">
            <a href="{{ route('report.revision-table', $record->id) }}" target="_blank" rel="noopener noreferrer" style="{{ $styBtnSecondary }}">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#ffffff" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M3.375 19.5h17.25m-17.25 0a1.125 1.125 0 01-1.125-1.125M3.375 19.5h7.5v-2.25m0 0h8.25m-8.25 0V12m0 5.25v-5.25m0 0h8.25m-8.25 0h-7.5m7.5 0v-2.25m0 2.25v2.25m0-2.25h-7.5m7.5 0h8.25"/></svg>
                Lembar Revisi
            </a>
            <a href="{{ route('report.revision-sign', $record->id) }}" target="_blank" rel="noopener noreferrer" style="{{ $styBtnSecondary }}">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#ffffff" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg>
                Keterangan Revisi
            </a>
            @if ($record->exam_type_id == 3)
                <a href="{{ route('report.thesis-exam-chief', $record->id) }}" target="_blank" rel="noopener noreferrer" style="{{ $styBtnThesis }}">BA Hasil Ujian</a>
                <a href="{{ route('report.thesis-exam-by-lecture', $record->id) }}" target="_blank" rel="noopener noreferrer" style="{{ $styBtnThesis }}">Penilaian by Penguji</a>
                <a href="{{ route('report.thesis-rev-by-lecture', $record->id) }}" target="_blank" rel="noopener noreferrer" style="{{ $styBtnThesis }}">Revisi by Penguji</a>
            @endif
        </div> --}}
    </div>
</div>
                    ...
                    <div class="flex items-center gap-1.5 flex-wrap">
                        @if ($waUrl)
                            <a href="{{ $waUrl }}" target="_blank"
                               style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:6px;color:#16a34a;text-decoration:none;flex-shrink:0;"
