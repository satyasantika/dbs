                        ...
                        <table class="table small-font table-sm" id="profile-table" role="grid">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        @php
                                            $exam_scores = \App\Models\ExamScore::with(['lecture','registration.student','registration.examtype'])->whereNull('pass_approved')->where('exam_registration_id',$exam_registration->id)->get();
                                        @endphp
                                        ...
                                        <a target="_blank" href="{{'https://api.whatsapp.com/send/?phone=62'
                                            ...
                                            .($exam_score->registration->student->name ?? '-').',%0A%0AMohon%20segera%20memberikan%20penilaian%20'
                                            ...
                                            class="badge rounded-pill bg-primary btn btn-sm">wa: {{ $exam_score->lecture->name ?? '-' }}</a>
                                        @endforeach
                                    </td>
                                    <td>{{ $exam_registration->student->name ?? '-' }}</td>
