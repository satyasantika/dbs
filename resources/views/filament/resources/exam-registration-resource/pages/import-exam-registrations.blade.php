            ...
            <table style="width:100%;border-collapse:collapse;font-size:12px;">
                <thead id="previewHead" style="position:sticky;top:0;background:#1f2937;color:#fff;">
                    <tr>
                        ...
                        <th style="padding:7px 10px;white-space:nowrap;">Status</th>
                    </tr>
                </thead>
...
(function () {
    ...
    window.doParse = function () {
        ...
        var headerKeys = hasHeader
            ? lines[0].split('\t').map(function (h) { return h.trim().toLowerCase().replace(/[\s.]+/g, '_'); })
            : null;
        ...
        var modeTxt    = hasHeader ? 'mode: dengan header' : 'mode: tanpa header (posisi tetap)';
        var modeBg     = hasHeader ? '#dbeafe' : '#f3f4f6';
        var modeColor  = hasHeader ? '#1d4ed8' : '#374151';
        ...
        document.getElementById('summaryBanner').innerHTML =
            ...
            '<strong style="color:#15803d">' + validCount + ' siap disimpan</strong>' +
        ...
        rows.forEach(function (row) {
            ...
            var badgeTxt   = row._invalid ? ('Kurang: ' + row._missing.join(', ')) : 'Siap';
            var badgeStyle = row._invalid
            ...
            tr.innerHTML =
                ...
                '<td style="padding:5px 10px;font-family:monospace;border-bottom:1px solid #f3f4f6;">' + (row.npm || '-') + '</td>' +
                '<td style="padding:5px 10px;border-bottom:1px solid #f3f4f6;">' + (row.nama_mahasiswa || '—') + '</td>' +
                '<td style="padding:5px 10px;border-bottom:1px solid #f3f4f6;">' + (row.jenis_ujian || '-') + '</td>' +
                '<td style="padding:5px 10px;white-space:nowrap;border-bottom:1px solid #f3f4f6;">' + (row.tanggal_ujian || '-') + '</td>' +
                '<td style="padding:5px 10px;white-space:nowrap;border-bottom:1px solid #f3f4f6;">' + (row.ruang || '-') + (row.waktu ? ' · ' + row.waktu : '') + '</td>' +
                '<td style="padding:5px 10px;border-bottom:1px solid #f3f4f6;"><span style="' + badgeStyle + '">' + badgeTxt + '</span></td>';
            tbody.appendChild(tr);
        });

        document.getElementById('commitNote').textContent =
            validCount === 0 ? 'Tidak ada baris valid untuk disimpan.' : validCount + ' baris akan disimpan.';

        resetCommitButton();
        var btnCommit = document.getElementById('btnCommit');
        btnCommit.disabled = validCount === 0;
        btnCommit.style.opacity = validCount === 0 ? '0.5' : '1';
        btnCommit.style.cursor = validCount === 0 ? 'not-allowed' : 'pointer';
    ...
    window.doCommit = function () {
        var validRows = parsedRows.filter(function (r) { return !r._invalid; });
