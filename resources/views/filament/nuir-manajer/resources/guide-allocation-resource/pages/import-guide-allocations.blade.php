...
<div id="import-page" wire:ignore style="font-size:14px;max-width:900px;">
    ...
    <div id="step1">
        ...
        <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:16px 20px;margin-bottom:20px;">
            ...
            <ol style="margin:0;padding-left:22px;color:#166534;line-height:2;">
                <li>Buka data kuota pembimbing di <strong>Google Sheets</strong> atau <strong>Excel</strong></li>
        ...
        <div style="background:#fafafa;border:1px solid #e5e7eb;border-radius:8px;padding:16px 20px;margin-bottom:20px;">
            ...
            <div style="overflow-x:auto;">
                <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:600px;">
                    ...
                    <tbody>
                        ...
                        $cols = [
                            [1, 'no',        'no, no.',                          '1',           false],
                            [2, 'dosen',     'dosen, inisial, nama_dosen',         'SIN',         true],
                            [3, 'tahun',     'tahun, year, angkatan',              '2022',        true],
                            [4, 'kuota_p1',  'kuota_p1, p1, guide1_quota',         '3',           true],
                            [5, 'kuota_p2',  'kuota_p2, p2, guide2_quota',         '2',           true],
                            [6, 'aktif',     'aktif, active',                      'ya',          false],
                        ];
                        @endphp
            ...
            <div style="margin-top:10px;padding:8px 12px;background:#fffbeb;border:1px solid #fde68a;border-radius:6px;font-size:12px;color:#92400e;line-height:1.8;">
                ...
                • Kolom <strong>dosen</strong> diisi dengan <strong>inisial</strong> (contoh: SIN), <strong>username</strong> (contoh: pembimbing1), atau <strong>nama lengkap</strong> persis seperti di sistem.<br>
                • Kolom <strong>tahun</strong> adalah tahun angkatan mahasiswa (contoh: 2022, 2099).<br>
                • Kolom <strong>kuota_p1</strong> dan <strong>kuota_p2</strong> harus angka ≥ 0.<br>
                • Kolom <strong>aktif</strong> opsional — kosongkan untuk default <em>aktif</em>. Nilai yang diterima: ya/tidak, 1/0, aktif.<br>
                • Jika kuota dosen+tahun sudah ada, baris akan <strong>diperbarui otomatis</strong> saat simpan (klik Batalkan jika tidak ingin mengubah).<br>
                • Kuota baru tidak boleh lebih kecil dari jumlah yang sudah terisi (Terisi P1/P2).
            </div>
        </div>
            ...
            <table style="width:100%;border-collapse:collapse;font-size:12px;">
                <thead id="previewHead" style="position:sticky;top:0;background:#1f2937;color:#fff;">
                    <tr>
                        ...
                        <th style="padding:7px 10px;white-space:nowrap;">Dosen</th>
                        <th style="padding:7px 10px;white-space:nowrap;">Tahun</th>
                        <th style="padding:7px 10px;white-space:nowrap;">Kuota P1</th>
                        <th style="padding:7px 10px;white-space:nowrap;">Kuota P2</th>
                        <th style="padding:7px 10px;white-space:nowrap;">Aktif</th>
    ...
    <div id="step3" style="display:none;">
        ...
        <div style="display:flex;gap:12px;margin-top:16px;flex-wrap:wrap;">
            ...
            <a
                href="{{ \App\Filament\NuirManajer\Resources\GuideAllocationResource::getUrl('index') }}"
...
(function () {
    ...
    var STATE = window.guideAllocPasteImport = window.guideAllocPasteImport || {};
    ...
    var POSITIONAL_FIELDS = ['dosen', 'tahun', 'kuota_p1', 'kuota_p2', 'aktif'];

    var HEADER_MAP = {
        'no': null, 'no.': null,
        'dosen': 'dosen', 'inisial': 'dosen', 'nama_dosen': 'dosen',
        'tahun': 'tahun', 'year': 'tahun', 'angkatan': 'tahun',
        'kuota_p1': 'kuota_p1', 'p1': 'kuota_p1', 'guide1_quota': 'kuota_p1',
        'kuota_p2': 'kuota_p2', 'p2': 'kuota_p2', 'guide2_quota': 'kuota_p2',
        'aktif': 'aktif', 'active': 'aktif',
    };

    var REQUIRED = ['dosen', 'tahun', 'kuota_p1', 'kuota_p2'];
    ...
    function formatActive(value) {
        if (!value || String(value).trim() === '') return 'ya (default)';
    ...
    function renderPreview() {
        ...
        rows.forEach(function (row) {
            ...
            tr.innerHTML =
                ...
                '<td style="padding:5px 10px;font-family:monospace;border-bottom:1px solid #f3f4f6;">' + escapeHtml(row.dosen || '-') + '</td>' +
                '<td style="padding:5px 10px;border-bottom:1px solid #f3f4f6;">' + escapeHtml(row.tahun || '-') + '</td>' +
                '<td style="padding:5px 10px;border-bottom:1px solid #f3f4f6;">' + escapeHtml(row.kuota_p1 || '-') + '</td>' +
                '<td style="padding:5px 10px;border-bottom:1px solid #f3f4f6;">' + escapeHtml(row.kuota_p2 || '-') + '</td>' +
                '<td style="padding:5px 10px;border-bottom:1px solid #f3f4f6;">' + formatActive(row.aktif) + '</td>' +
    ...
    function fetchDuplicateChecks(rows) {
        ...
        return fetch('{{ route('guideallocations.paste-import-check-duplicates') }}', {
            method: 'POST',
            headers: {
    ...
    window.doParse = function () {
        ...
        var rows = dataLines.map(function (line, i) {
            ...
            row._dupInfo = null;
            return row;
        }).filter(function (r) { return r.dosen || r.tahun; });
    ...
    window.doCommit = function () {
        ...
        fetch('{{ route('guideallocations.paste-import') }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
